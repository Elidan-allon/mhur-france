(()=>{
  ...
  function arrangeMobile(header){
    ...
    header.classList.add('mhurMobileHeaderV511','mhurHeaderReadyV511');
  ...
  function arrangeDesktop(header){
    ...
    toolbar?.remove();
    brandRow?.remove();
    header.classList.remove('mhurMobileHeaderV511');
    header.classList.add('mhurHeaderReadyV511');
    ROOT.style.setProperty('--mhur-top-height','72px');
    ROOT.style.setProperty('--mhur-header-bottom','72px');
  }

  function measure(header=getHeader()){
    if(!header)return;
    requestAnimationFrame(()=>{
      ...
      ROOT.style.setProperty('--mhur-header-bottom',`${Math.max(height,Math.ceil(rect.bottom))}px`);
    });
  }
  ...
  function mountObservers(){
    ...
    if(header){
      ...
      if('ResizeObserver' in window){
        ...
        resizeObserver=new ResizeObserver(()=>measure(header));
    ...
    bodyObserver=new MutationObserver(()=>{
      if(getHeader()&&!getHeader().classList.contains('mhurHeaderReadyV511'))schedule();
      if(getDrawer()&&!drawerObserver)mountObservers();
    });
    bodyObserver.observe(document.body,{childList:true,subtree:false});
  }

  /* Fermer un panneau en touchant vraiment à côté, y compris le menu principal. */
  ...
  /* Le fichier est placé juste après le header : le premier rendu est déjà rangé. */
  mount();
