...
function footer(){
  ...
  const labels=isEnglish()
    ?{privacy:'Privacy',rules:'Rules',about:'About',notice:`Unofficial community project · Version V${RELEASE}`}
    :{privacy:'Confidentialité',rules:'Règles',about:'À propos',notice:`Projet communautaire non officiel · Version V${RELEASE}`};
  footerElement.querySelectorAll('[data-legal]').forEach(button=>{
    button.textContent=labels[button.dataset.legal]||'';
  });
  const notice=footerElement.querySelector('span');
