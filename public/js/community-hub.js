...
// Comments — texte, photo ou vidéo + date et heure
...
const comments={
  buildId:null,
  async mount(buildId){this.buildId=String(buildId);const content=document.getElementById('cbDetailContent');if(!content)return;content.querySelector('.mhurCommentBox')?.remove();content.insertAdjacentHTML('beforeend',`<section class="mhurCommentBox"><h3>💬 ${t('Commentaires','Comments')}</h3><div class="mhurCommentComposer"><textarea id="mhurCommentText" class="mhurHubInput" maxlength="700" placeholder="${t('Écris un commentaire…','Write a comment…')}"></textarea><div class="mhurCommentTools"><label class="mhurCommentFile">📷 ${t('Photo / vidéo','Image / video')}<input id="mhurCommentFile" type="file" accept="image/*,video/*"></label><span id="mhurCommentFileName"></span><progress id="mhurCommentProgress" max="100" value="0" hidden></progress><button onclick="MHUR_HUB.comments.send()">${t('Publier','Post')}</button></div></div><div id="mhurCommentList" class="mhurCommentList"><div>${t('Chargement…','Loading…')}</div></div></section>`);const f=document.getElementById('mhurCommentFile');f?.addEventListener('change',()=>{document.getElementById('mhurCommentFileName').textContent=f.files?.[0]?.name||''});await this.load()},
  async load(){const out=document.getElementById('mhurCommentList');if(!out)return;if(!remote){out.innerHTML=`<div>${t('Configure Supabase pour activer les commentaires.','Configure Supabase to enable comments.')}</div>`;return}try{const rows=await req(`/rest/v1/community_build_comments?build_id=eq.${encodeURIComponent(this.buildId)}&is_hidden=eq.false&select=*,profile:profiles(id,username,avatar_url,role)&order=created_at.asc`);out.innerHTML=(rows||[]).map(c=>{const p=c.profile||{},mine=user()?.id===c.user_id,admin=window.MHUR_MODERATION?.isAdmin?.();const av=p.avatar_url?`<img class="mhurCommentAvatar" src="${esc(p.avatar_url)}">`:`<span class="mhurCommentAvatar">${esc((p.username||'?').slice(0,2).toUpperCase())}</span>`;return `<article class="mhurComment"><div class="mhurCommentHead">${av}<b>${esc(p.username||t('Utilisateur','User'))}</b><small>🕒 ${commentDate(c.created_at)}</small>${mine||admin?`<button class="mhurCommentDelete" onclick="MHUR_HUB.comments.remove('${c.id}','${esc(c.media_path||'')}')">×</button>`:''}</div>${c.content?`<p>${esc(c.content)}</p>`:''}${commentMediaHtml(c)}</article>`}).join('')||`<div>${t('Aucun commentaire.','No comments yet.')}</div>`}catch(e){out.textContent=e.message}},
  async send(){if(!window.MHUR_AUTH?.requireLogin?.(t('Connecte-toi pour commenter.','Sign in to comment.')))return;const input=document.getElementById('mhurCommentText'),fileInput=document.getElementById('mhurCommentFile'),progress=document.getElementById('mhurCommentProgress'),content=String(input?.value||'').trim(),file=fileInput?.files?.[0];if(!content&&!file)return;let media=null;try{if(file){progress.hidden=false;media=await uploadCommentMedia(file,`build-${this.buildId}`,pct=>progress.value=pct)}await req('/rest/v1/community_build_comments',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({build_id:this.buildId,user_id:user().id,content,...(media||{})})});input.value='';if(fileInput)fileInput.value='';await this.load()}catch(e){if(media?.media_path)await deleteCommentMedia(media.media_path);alert(e.message)}finally{if(progress)progress.hidden=true}},
  async remove(id,path=''){if(!confirm(t('Supprimer ce commentaire ?','Delete this comment?')))return;try{await req(`/rest/v1/community_build_comments?id=eq.${encodeURIComponent(id)}`,{method:'DELETE'});await deleteCommentMedia(path);await this.load()}catch(e){alert(e.message)}}
};
