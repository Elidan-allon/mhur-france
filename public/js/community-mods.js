...
const state={rows:[],profiles:{},liked:new Set(),loading:false,error:'',category:'all',character:'all',query:'',sort:'recent',active:null,comments:[],editingId:null};
...
function bindPage(){document.querySelectorAll('.modsCopyCommand').forEach(b=>b.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(b.dataset.copy||'-fileopenlog');const span=b.querySelector('span');if(span){const old=span.textContent;span.textContent=tx('Copié !','Copied!');setTimeout(()=>span.textContent=old,1400)}}catch(_){prompt(tx('Copie cette commande :','Copy this command:'),b.dataset.copy||'-fileopenlog')}}));document.getElementById('modsPublishBtn')?.addEventListener('click',openPublish);document.getElementById('modsSearch')?.addEventListener('input',e=>{state.query=e.target.value;renderPage()});document.getElementById('modsCategory')?.addEventListener('change',e=>{state.category=e.target.value;renderPage()});document.getElementById('modsCharacter')?.addEventListener('change',e=>{state.character=e.target.value;renderPage()});document.getElementById('modsSort')?.addEventListener('change',e=>{state.sort=e.target.value;renderPage()});document.querySelectorAll('[data-mod-open]').forEach(b=>b.addEventListener('click',()=>openDetail(b.dataset.modOpen)));document.querySelectorAll('[data-mod-tag]').forEach(b=>b.addEventListener('click',()=>{const t=b.dataset.modTag;if(CATEGORIES.includes(t.toLowerCase()))state.category=t.toLowerCase();else state.character=t;renderPage()}))}
...
function closeModsModal(modal){
  if(!modal)return;
  modal.hidden=true;
  if(modal.id==='modsPublishModal')state.editingId=null;
  if(!document.querySelector('.modsModal:not([hidden]),.cbModal.open,.mhurHubOverlay.open,.mhurAuthOverlay.open,.mhurPublicProfileModal.open,.mhurModerationOverlay.open'))document.body.classList.remove('cbModalOpen');
}
function deleteStoredObject(bucket,path){
  if(!path)return Promise.resolve();
  return request(`/storage/v1/object/${bucket}`,{method:'DELETE',body:JSON.stringify({prefixes:[path]})}).catch(()=>null);
}
function setPublishMode(mode,row=null){
  const m=ensurePublishModal(),f=m.querySelector('#modsForm');
  f.reset();
  f.dataset.mode=mode;
  f.dataset.editId=row?.id||'';
  state.editingId=row?.id||null;
  const editing=mode==='edit';
  m.querySelector('#modsFormTitle').textContent=editing?tx('Modifier le mod','Edit mod'):tx('Publier un mod','Publish a mod');
  f.querySelector('.modsSubmit').textContent=editing?tx('Enregistrer les modifications','Save changes'):tx('Publier','Publish');
  f.mod_file.required=!editing;
  f.preview_file.required=!editing;
  f.querySelector('[data-current-mod]').textContent=editing?`${tx('Fichier actuel','Current file')}: ${row?.file_name||''}`:'';
  f.querySelector('[data-current-preview]').textContent=editing?tx('Laisse vide pour garder la présentation actuelle.','Leave empty to keep the current preview.'):'';
  if(editing&&row){
    f.title.value=row.title||'';
    f.description.value=row.description||'';
    f.category.value=normalizedCategory(row.category);
    f.mod_version.value=row.mod_version||'1.0';
    f.game_version.value=row.game_version||'';
    f.character_id.value=row.character_id||'';
    f.tags.value=Array.isArray(row.tags)?row.tags.join(', '):'';
  }else{
    f.mod_version.value='1.0';
  }
  return m;
}
...
function ensurePublishModal(){
  ...
  m.hidden=true;
  m.innerHTML=`<div class="modsDialog"><div class="modsDialogHead"><h2 id="modsFormTitle">${tx('Publier un mod','Publish a mod')}</h2><button class="modsClose" type="button" aria-label="${tx('Fermer','Close')}">×</button></div><form class="modsForm" id="modsForm"><label>${tx('Nom du mod','Mod name')}<input name="title" maxlength="100" required></label><label>${tx('Description','Description')}<textarea name="description" maxlength="3000" required></textarea></label><div class="modsFormGrid"><label>${tx('Catégorie','Category')}<select name="category" required>${CATEGORIES.map(c=>`<option value="${c}">${categoryLabel(c)}</option>`).join('')}</select></label><label>${tx('Version du mod','Mod version')}<input name="mod_version" value="1.0" maxlength="30"></label><label>${tx('Version du jeu','Game version')}<input name="game_version" placeholder="Saison 18 / Season 18" maxlength="50"></label></div><label class="modsCharacterField">${tx('Personnage concerné','Related character')}<select name="character_id"><option value="">${tx('Aucun / général','None / general')}</option>${characterOptions()}</select></label><label>${tx('Tags supplémentaires','Additional tags')}<input name="tags" placeholder="HUD, recolor, voice…"><span class="modsFieldHint">${tx('Sépare les tags avec des virgules.','Separate tags with commas.')}</span></label><label>${tx('Fichier du mod','Mod file')}<input name="mod_file" type="file"><span class="modsFieldHint">.pak, .utoc, .ucas ou .zip — 200 MB maximum. ${tx('Sur iPhone, le sélecteur affiche maintenant tous les fichiers.','On iPhone, the picker now shows all files.')}</span><span class="modsFieldHint modsCurrentFile" data-current-mod></span></label><label>${tx('Image ou vidéo de présentation','Preview image or video')}<input name="preview_file" type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm"><span class="modsFieldHint">JPG, PNG, WEBP, GIF, MP4 ou WEBM — 20 MB maximum</span><span class="modsFieldHint modsCurrentFile" data-current-preview></span></label><button class="modsSubmit" type="submit">${tx('Publier','Publish')}</button><div class="modsProgress" id="modsProgress"></div></form></div>`;
  document.body.appendChild(m);
  m.querySelector('.modsClose').onclick=()=>closeModsModal(m);
  m.addEventListener('click',e=>{if(e.target===m)closeModsModal(m)});
...
function openPublish(){
  ...
  const m=setPublishMode('create');
  m.hidden=false;
  document.body.classList.add('cbModalOpen');
  m.querySelector('.modsDialog')?.scrollTo({top:0,behavior:'auto'});
}
async function publish(e){
  e.preventDefault();
  const f=e.currentTarget,u=user(),mod=f.mod_file.files[0],pre=f.preview_file.files[0],submit=f.querySelector('.modsSubmit'),progress=f.querySelector('#modsProgress');
  if(!u)return window.MHUR_AUTH?.open?.();
  if(!REMOTE)return alert(tx('Supabase n’est pas configuré.','Supabase is not configured.'));
  const editing=f.dataset.mode==='edit';
  const existing=editing?state.rows.find(r=>String(r.id)===String(f.dataset.editId)):null;
  if(editing&&(!existing||String(existing.creator_id)!==String(u.id)))return alert(tx('Tu ne peux modifier que tes propres mods.','You can only edit your own mods.'));
  if(!editing&&(!mod||!pre))return alert(tx('Ajoute le fichier du mod et une image ou vidéo de présentation.','Add the mod file and a preview image or video.'));
  if(mod){
    const ext=(mod.name.match(/\.[^.]+$/)?.[0]||'').toLowerCase();
    if(!['.pak','.utoc','.ucas','.zip'].includes(ext))return alert(tx('Format de mod non accepté.','Unsupported mod format.'));
    if(mod.size>200*1024*1024)return alert(tx('Le fichier dépasse 200 MB.','The file exceeds 200 MB.'));
  }
  if(pre&&pre.size>20*1024*1024)return alert(tx('La présentation dépasse 20 MB.','The preview exceeds 20 MB.'));
  submit.disabled=true;
  try{
    const id=editing?existing.id:crypto.randomUUID();
    const selected=f.character_id.selectedOptions[0];
    const payload={title:f.title.value.trim(),description:f.description.value.trim(),category:f.category.value,character_id:f.character_id.value||null,character_name:f.character_id.value?(selected?.dataset.name||selected?.textContent):null,game_version:f.game_version.value.trim()||null,mod_version:f.mod_version.value.trim()||'1.0',tags:f.tags.value.split(',').map(x=>x.trim()).filter(Boolean).slice(0,10)};
    let oldFilePath='',oldPreviewPath='';
    if(mod){
      const filePath=`${u.id}/${id}/${Date.now()}_${safeName(mod.name)}`;
      progress.textContent=tx('Envoi du mod…','Uploading mod…');
      const fileUrl=await upload('community-mods',filePath,mod);
      Object.assign(payload,{file_url:fileUrl,file_path:filePath,file_name:mod.name,file_size:mod.size});
    ...
    if(pre){
      ...
      const previewUrl=await upload('mod-previews',previewPath,pre);
      Object.assign(payload,{preview_url:previewUrl,preview_path:previewPath,preview_type:pre.type.startsWith('video/')?'video':'image'});
      oldPreviewPath=existing?.preview_path||'';
    }
    progress.textContent=editing?tx('Enregistrement…','Saving…'):tx('Publication…','Publishing…');
    if(editing){
      await request(`/rest/v1/community_mods?id=eq.${encodeURIComponent(id)}&creator_id=eq.${encodeURIComponent(u.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(payload)});
      await Promise.all([oldFilePath?deleteStoredObject('community-mods',oldFilePath):null,oldPreviewPath?deleteStoredObject('mod-previews',oldPreviewPath):null].filter(Boolean));
    }else{
      await request('/rest/v1/community_mods',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({id,creator_id:u.id,...payload,is_hidden:false})});
    }
    closeModsModal(ensurePublishModal());
    closeModsModal(document.getElementById('modsDetailModal'));
    f.reset();
    await load();
  }catch(err){
...
function openEdit(id){
  const row=state.rows.find(r=>String(r.id)===String(id)),u=user();
  if(!row||!u||String(row.creator_id)!==String(u.id))return;
  closeModsModal(document.getElementById('modsDetailModal'));
  const m=setPublishMode('edit',row);
  m.hidden=false;
  document.body.classList.add('cbModalOpen');
  m.querySelector('.modsDialog')?.scrollTo({top:0,behavior:'auto'});
}
...
async function openDetail(id){
  ...
  const p=state.profiles[r.creator_id]||{},mine=String(user()?.id||'')===String(r.creator_id||''),liked=state.liked.has(String(id));
...
async function removeMod(id){
  ...
  try{
    ...
    await Promise.all([deleteStoredObject('community-mods',row.file_path),deleteStoredObject('mod-previews',row.preview_path)]);
    closeModsModal(document.getElementById('modsDetailModal'));
    await load();
