...
function card(r){
  ...
  return `<article class="modCard" data-mod-id="${esc(r.id)}">
    ...
    <div class="modBody">
      ...
      <div class="modMeta">${esc(p.username||tx('Membre','Member'))} · ${formatDate(r.created_at)} · ${formatSize(r.file_size)}</div>
      <div class="modStats"><span>♥ ${Number(r.likes_count)||0}</span><span>⬇ ${Number(r.downloads_count)||0}</span></div>
...
async function publish(e){
  ...
  try{
    ...
    if(editingId){
      ...
      if(mod&&oldRow?.file_path&&oldRow.file_path!==payload.file_path)try{await deleteStorageObject('community-mods',oldRow.file_path)}catch(_){}
...
async function removeMod(id){
  ...
  try{
    ...
    for(const [bucket,path] of [['community-mods',row.file_path],['mod-previews',row.preview_path]])if(path)try{await deleteStorageObject(bucket,path)}catch(_){}
    document.getElementById('modsDetailModal')?.setAttribute('hidden','');
    await load();
...
window.addEventListener('mhur-auth-change',()=>{loadLikes().finally(()=>{renderPage();const detail=document.getElementById('modsDetailModal');if(detail&&!detail.hidden&&state.active)openDetail(state.active.id)})});
