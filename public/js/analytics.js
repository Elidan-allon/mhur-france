...
function pageTitle(){return document.title||'MHUR France'}
...
function banner(){if(localStorage.getItem(KEY))return;const el=document.createElement('div');el.id='mhurAnalyticsConsent';el.innerHTML='<div><b>Statistiques anonymes</b><span>Autoriser Google Analytics pour nous aider à améliorer MHUR France.</span></div><div><button data-no>Refuser</button><button data-yes>Accepter</button></div>';document.body.appendChild(el);el.querySelector('[data-no]').onclick=()=>MHUR_ANALYTICS.consent(false);el.querySelector('[data-yes]').onclick=()=>MHUR_ANALYTICS.consent(true);}
