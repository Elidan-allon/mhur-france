/* ============================================================
   MHUR FRANCE — V533
   1. Recentre automatiquement le sujet des portraits.
   2. Gère le scroll selon la navigation :
      - changement de catégorie => tout en haut ;
      - entrée dans une fiche => tout en haut ;
      - retour à la liste => ancienne position restaurée ;
      - nouveau rendu de la même vue => position conservée.
   ============================================================ */
(() => {
  "use strict";
  ...
  const IMAGE_SELECTOR = [
    ...
    "#app .styleGrid .styleBanner > img",
    "#app .charPanel .portrait > img"
  ].join(",");

  const CENTER_CACHE_PREFIX = "mhur_v533_center:";
  const SCROLL_CACHE_KEY = "mhur_v533_scroll_positions";
  ...
  function analysePortrait(image) {
    ...
    /*
      Les portraits officiels placent généralement le personnage
      vers la droite du fichier. On déplace seulement ce qu'il faut
      pour amener son centre visuel au milieu du carré.
    */
  ...
  async function preparePortrait(image) {
    ...
    image.classList.add("v533-portrait-image");
    ...
    image.style.setProperty("--v533-shift-x", `${result.shift}%`);
  ...
  function installNavigationRender() {
    if (
      ...
      window.render.__mhurV533Navigation
    ) {
      return;
    }
    ...
    const smartRender = function () {
      ...
      const categoryChanged =
        previousState.page !== nextState.page;
      const exactSameView =
        stateSignature(previousState) ===
      ...
      /*
        Empêche render() de remettre momentanément la page tout en haut
        quand nous devons conserver ou restaurer une position.
      */
    ...
    smartRender.__mhurV533Navigation = true;
  ...
  function installCategoryNavigationGuard() {
    if (
      ...
      window.go.__mhurV533Navigation
    ) {
      return;
    }
    ...
    smartGo.__mhurV533Navigation = true;
    window.go = smartGo;
  ...
  function install() {
    ...
    if (!document.documentElement.dataset.v533PortraitObserver) {
      document.documentElement.dataset.v533PortraitObserver = "1";

      /*
        Cet observateur ne rappelle jamais render().
        Il prépare uniquement les nouvelles images insérées.
      */
    ...
    document.documentElement.classList.add("mhur-v533-ready");
    console.info("[MHUR] Portraits et navigation V533 actifs.");
  }
