(function () {
  ...
  var cssTag =
    '<link rel="stylesheet" href="css/v549-mods-single-arrow.css?v=549">';

  var jsTag =
    '<script src="js/v549-mods-single-arrow.js?v=549"></script>';
  ...
  try {
    log("");
    log("============================================");
    log("  MHUR FRANCE - CORRECTIF V549");
    log("============================================");
    log("");
    ...
    if (!fso.FileExists(cssPath)) {
      fail("Le CSS V549 est introuvable.");
    }

    if (!fso.FileExists(jsPath)) {
      fail("Le JavaScript V549 est introuvable.");
    }
    ...
    if (
      ...
      fail(
        "Le CSS V549 n'est pas chargé exactement une fois."
      );
    }
    ...
    if (
      ...
      fail(
        "Le JavaScript V549 n'est pas chargé exactement une fois."
      );
    }

    log("[OK] Les anciennes fleches du tutoriel sont retirees.");
    log("[OK] Une seule fleche reste a droite.");
    log("[OK] Le V547 et le V548 restent actifs.");
    log("");
    log("TOUTES LES VERIFICATIONS V549 SONT BONNES");
    log("");
    log(
