...
const REPORT = path.join(ROOT, 'RAPPORT_V577.txt');

const MARKER_JS = 'MHUR Nexus — V577 : NEW source propre';
const MARKER_CSS = 'MHUR Nexus — V577 : position et animation finales';
...
function patchFixesJs() {
  ...
  if (!source.includes('V577 preserve data-costume')) {
...
(function(){
  ...
  const BADGE='<span class="s18NewBadge s18NewBadgeV9 s18NewBadgeV24 s18NewBadgeV577" aria-label="NEW">NEW!</span>';
  ...
  function setBadgeV577(node,active){
    if(!node)return;
  ...
  function wrapRenderV577(){
    if(typeof window.render!=='function'||window.render.__mhurV577)return;
  ...
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',scheduleV577,{once:true});
  }else{
    scheduleV577();
  }
  ...
  window.addEventListener('load',scheduleV577,{once:true});
  window.addEventListener('hashchange',scheduleV577);
  window.addEventListener('mhur:languagechange',scheduleV577);
  window.MHUR_V577_NEW={refresh:syncV577,latestCostumes:latestCostumesV577};
})();
`;
...
function patchEarlyJs() {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    if (lines[i].includes('data-planned="twice"')) {
      lines[i] = lines[i].replace(
        '<span class="s18PlannedNewV12"></span>',
        '<span class="s18PlannedIncomingV577">INCOMING</span>'
      );
      twice = lines[i].includes('s18PlannedIncomingV577');
    }

    if (lines[i].includes('data-planned="tsuyu"')) {
      lines[i] = lines[i].replace(
        '<span class="s18PlannedNewV12"></span>',
        '<span class="s18PlannedIncomingV577">INCOMING</span>'
      );
      tsuyu = lines[i].includes('s18PlannedIncomingV577');
    }
  }

  if (!twice || !tsuyu) {
    fail('Impossible de remplacer les badges de Twice/Tsuyu dans season18-early.js.');
  }

  fs.writeFileSync(EARLY_JS, lines.join('\n') + '\n', 'utf8');
...
function patchIndex() {
  ...
  console.log('[CORRIGÉ] Cache forcé en V577 et anciens chargements V572/V573 retirés.');
}
...
function verify(info) {
  ...
  for (const ref of [
    'css/season18-fixes.css?v=577',
    'data/season18_sync.js?v=577',
    'js/season18-early.js?v=577',
    'js/season18-fixes.js?v=577'
  ]) {
    if (!html.includes(ref)) errors.push(`cache V577 absent : ${ref}`);
  }
  ...
  if (!fixesJs.includes(MARKER_JS)) errors.push('bloc JavaScript V577 absent');
  if (!fixesJs.includes('V577 preserve data-costume')) errors.push('data-costume non conservé');
  if (fixesJs.includes('MHUR V574 — charge V573 sans index.html')) errors.push('chargeur V574 encore présent');
  if (!fixesCss.includes(MARKER_CSS)) errors.push('bloc CSS V577 absent');

  if (!early.includes('data-planned="twice"') || !early.includes('s18PlannedIncomingV577')) {
    errors.push('INCOMING de Twice absent');
  }
  if (!early.includes('data-planned="tsuyu"') || (early.match(/s18PlannedIncomingV577/g) || []).length < 2) {
  ...
  if (errors.length) {
    fail('Vérification V577 échouée :\n- ' + errors.join('\n- '));
  }
  ...
  console.log('[VÉRIFIÉ] Cache V577 et syntaxe JavaScript : OK');
}
...
function main() {
  ...
  console.log('\n=== MHUR FRANCE — CORRECTIF SOURCE V577 ===\n');
  ...
  try {
    ...
    console.log('\n[OK] Correctif V577 appliqué et vérifié.');
    console.log('[OK] Rapport créé : RAPPORT_V577.txt');
    console.log('[SUITE] Envoie seulement les 5 fichiers modifiés indiqués dans LISEZ-MOI_V577.txt.\n');
  } catch (error) {
    rollback();
    throw error;
  }
...
try {
  ...
  console.error('\n[ERREUR V577] ' + (error && error.message ? error.message : String(error)));
